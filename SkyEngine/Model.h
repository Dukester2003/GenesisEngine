...
#include "src/Scene/Animation/AnimationData.h"
...
private:
    ...
    unsigned int EmbeddedTexture(aiTexture* ai_texture, bool gamma = false)
    {
        ...
        unsigned char* data = stbi_load_from_memory(reinterpret_cast<stbi_uc*>(ai_texture->pcData), ai_texture->mWidth, &width, &height, &nrComponents, 0);
        if (data)
        {
            GLenum format;
            if (nrComponents == 1)
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        {   // if texture hasn't been loaded already, load it
            Texture texture;
            if(str.C_Str()[0] == '*') {
                int index = std::stoi(str.C_Str() + 1);
                aiTexture* embedded_texture = scene->mTextures[index];
                texture.id = EmbeddedTexture(embedded_texture);
                texture.type = typeName;
                texture.path = str.C_Str();
                textures.push_back(texture);
            } else {
                // The path is a regular file path, load the texture from the file
